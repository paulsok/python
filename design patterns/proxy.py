    ...
    def highest_total_price_reservations(self, date_from, date_to, reservations_count):
        if self.current_user.can_see_reservations:
            return self.reservation_service.\
                        highest_total_price_reservations(date_from, date_to, reservations_count)
        else:
            return []
...
class ReservationService:
    def highest_total_price_reservations(self, date_from, date_to, reservations_count):
        ...
        reservations = [Reservation(date(2014, 5, 15), 100),
...
class StatsService:
    ...
    def year_top_100_reservations_average_total_price(self, year):
        reservations = self.reservation_service.\
                            highest_total_price_reservations(date(year, 1, 1), date(year, 12, 31), 1)
